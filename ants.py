...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        a = self.min_range
        while (current_place != hive or current_place.entrance != None) and a <= self.max_range:
...
# END Problem 13
    ...
    def __init__(self):
        ...
        QueenAnt.how_many_queen += 1
        if QueenAnt.how_many_queen > 1:
            self.imposter = True
        # END Problem 13
    ...
    def action(self, colony):
        ...
        You can find each Place in a tunnel behind the QueenAnt by starting
        at the ant's place.exit and then repeatedly following its exit.
        To detect whether a Place is at the end of a tunnel,
        check whether its exit is None.
        """
        # BEGIN Problem 13
        #real
        if self.imposter == False:
            ...
            #increaese damage of ants
            self.behind = []
            p = self.place.exit
            while p.exit != None:
                self.behind.append(p.ant)
                if p.ant and p.ant.container and p.ant.ant:
                    self.behind.append(p.ant.ant)
                p = p.exit
            for ant in self.behind:
                if ant not in self.ants_doubled and hasattr(ant, 'damage'):
                    ant.damage *= 2
                    self.ants_doubled.append(ant)
        #imposter
        elif self.imposter == True:
            self.armor = 0
            self.place.remove_insect(self)
...
def apply_effect(effect, bee, duration):
    ...
    cur_action = effect(bee.action)
    def apply(colony):
        nonlocal duration
        if duration == 0:
            return pre_action(colony)
        else:
            duration -= 1
            return cur_action(colony)
    bee.action = apply
    # END Problem EC
